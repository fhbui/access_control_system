...
lv_obj_t* ui_add_fp;
lv_obj_t* ui_msgbox = NULL;
#define UI_MSGBOX_H     80
#define UI_MSGBOX_W     200
lv_anim_timeline_t* anim_timeline = NULL;
...
void _ui_msgbox_popup(lv_obj_t * parent, char* text){
    ...
    // 避免重复创建
    if(ui_msgbox != NULL){
        lv_msgbox_close(ui_msgbox);     // 弹窗（或者class）不可用lv_mem_free
        ui_msgbox = NULL;
    }

    if(lv_scr_act() != parent){
        LOG_ERROR(TAG, "_ui_msgbox_popup : parent is not act");
        return ;
    }
...
void _ui_msgbox_close(void){
    if(ui_msgbox == NULL){
        LOG_ERROR(TAG, "_ui_msgbox_close : ui_msgbox is NULL in");
        return ;
    }
    if(anim_timeline != NULL){
        lv_anim_timeline_del(anim_timeline);
...
static void ui_event_keyboard_cb(lv_event_t * e)
{
    ...
    if(strcmp(text, LV_SYMBOL_BACKSPACE) == 0) {
		...
		if(screen_manager.page_id == UI_ENTER_PW_SCREEN){
            ...
            ui_msgbox_info_t info;
            info.has_close_msgbox = 1;
            info.target_scr = UI_CHANGE_PW_SCREEN;
            ...
            switch(change_state){
                ...
                case 2:     //比对新密码
                    if(enter_len == password_len_temp && memcmp(enterword, password_temp, password_len_temp) == 0){
                        ...
                        info.close_msgbox = 1;
                        info.ret_to_main = 1;
                        info.msg_in_box = "Change password success";
                        xQueueSend(queue_msgbox_info, &info, portMAX_DELAY);
                    }
                    else{
                        lv_textarea_set_text(textarea, "");
                        ...
                        info.close_msgbox = 1;
                        info.ret_to_main = 1;
                        info.msg_in_box = "error! please enter again";
                        xQueueSend(queue_msgbox_info, &info, portMAX_DELAY);
                    }
                    break;
            }
...
static lv_obj_t* scr_enum_to_obj(ui_screen_t scr_id){
    lv_obj_t* scr_obj;
    switch(scr_id){
        case UI_MAIN_SCREEN:
            scr_obj = ui_main;
            break;
        case UI_ENTER_PW_SCREEN:
            scr_obj = ui_enter_pw;
            break;
        case UI_CHANGE_PW_SCREEN:
            scr_obj = ui_change_pw;
            break;
        case UI_ADD_FP_SCREEN:
            scr_obj = ui_add_fp;
            break;
        case UI_DELETE_FP_SCREEN:
            scr_obj = ui_delete_fp;
            break;
        case UI_ADD_CARD_SCREEN:
            scr_obj = ui_add_card;
            break;
        case UI_DELETE_CARD_SCREEN:
            scr_obj = ui_delete_card;
            break;
        default:
            break;
    }
...
static void ui_timer_cb(lv_timer_t * user_data){
    ...
    static ui_msgbox_info_t info_recv;
    lv_obj_t* scr_obj;
    if(pdPASS == xQueueReceive(queue_msgbox_info, &info_recv, 0)){
        scr_obj = scr_enum_to_obj(info_recv.target_scr);
        _ui_msgbox_popup(scr_obj, info_recv.msg_in_box);
        timer_cnt = 0;
    }
    if(info_recv.close_msgbox == 1 && timer_cnt >= 15){
        _ui_msgbox_close();
        timer_cnt = 0;
		info_recv.close_msgbox = 0;

        // 回跳到主页
        if(info_recv.ret_to_main == 1){
            ui_screen_change(&ui_main, LV_SCR_LOAD_ANIM_FADE_ON, 200, 0, ui_main_screen_init);
        }
    }
    timer_cnt++;    //100ms一次
...
/*** 初始化函数 ***/
void ui_main_screen_init(void){
    fp_flag_t fp_state = FP_VERIFY;
	xQueueSend(xQueue_FPFlag, &fp_state, 10);
    pcd_flag_t pcd_state = PCD_CHECK_EXIST;
    xQueueSend(xQueue_PCDFlag, &pcd_state, 10);
