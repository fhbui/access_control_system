 ...
 * @param num   擦除扇区个数
 */
bsp_flash_status_t bsp_flash_erase_sector(uint32_t addr, uint32_t num){
    HAL_StatusTypeDef status; 
    uint32_t page_error = 0;
    uint32_t sector = bsp_flash_get_sector(addr);
...
bsp_flash_status_t bsp_flash_write(uint32_t addr, const uint8_t *pdata, uint32_t len) {
    ...
    for(uint32_t i=0; i<len; i++){
        status = HAL_FLASH_Program(FLASH_TYPEPROGRAM_BYTE, addr+i, (uint64_t)pdata[i]);
		if(status != HAL_OK){
			HAL_FLASH_Lock();
			return BSP_FLASH_ERROR;
		}
